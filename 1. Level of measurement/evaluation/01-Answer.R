  ...
  testcase("Feedback bij basisfuncties op df_crime_data", {
    ...
    testEqual(
      ...
      comparator = function(got, want, ...) {
        ...
        if (all(sapply(c("data.frame", "id", "type", "ernst", "age", "district"),
                       grepl, x=got))) {
          get_reporter()$add_message("✅ str(df_crime_data): toont kolomnamen, types en voorbeeldwaarden.", type="markdown")
          TRUE
        } else {
          get_reporter()$add_message("❌ str(df_crime_data): structuur niet zoals verwacht.", type="markdown")
          FALSE
        }
      }
    )
    ...
    testEqual(
      ...
      comparator = function(got, want, ...) {
        ...
        get_reporter()$add_message(paste0("```\n[1] ", got, "\n```"), type="markdown")
        if (got == want) {
          get_reporter()$add_message("✅ length(df_crime_data): aantal kolommen = 5.", type="markdown")
          TRUE
        } else {
          get_reporter()$add_message(paste0("❌ length(df_crime_data): geeft ", got, " in plaats van 5."), type="markdown")
          FALSE
        }
      }
    )
    ...
    testEqual(
      ...
      comparator = function(got, want, ...) {
        ...
        get_reporter()$add_message(paste0("```\n[1] ", got, "\n```"), type="markdown")
        if (got == want) {
          get_reporter()$add_message("✅ nrow(df_crime_data): aantal rijen = 5.", type="markdown")
          TRUE
        } else {
          get_reporter()$add_message(paste0("❌ nrow(df_crime_data): geeft ", got, " in plaats van 5."), type="markdown")
          FALSE
        }
      }
    )
    ...
    testEqual(
      ...
      comparator = function(got, want, ...) {
        ...
        get_reporter()$add_message(paste0("```\n[1] ", got, "\n```"), type="markdown")
        if (got == want) {
          get_reporter()$add_message("✅ ncol(df_crime_data): aantal kolommen = 5.", type="markdown")
          TRUE
        } else {
          get_reporter()$add_message(paste0("❌ ncol(df_crime_data): geeft ", got, " in plaats van 5."), type="markdown")
          FALSE
        }
      }
    )
