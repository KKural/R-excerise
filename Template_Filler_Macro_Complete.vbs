Sub FillExistingTemplate()
    ...
    templatePath = InputBox("Enter the full path to your PowerPoint template:", _
                           "Select Template", _
                           "C:\Users\kukumar\OneDrive - UGent\Job\Graffiti_location_choice.pptx")
    ...
    Do While pres.Slides.Count < 12  ' 11 main slides + 1 hidden slide
    ...
    Call FillSlideContent(slide, _
        ...
        "• Context rules: frequent reminders that correlation does not equal causation")
    ...
    Call FillSlideContent(slide, _
        ...
        "• swirl: step-by-step console lessons" & vbCrLf & _
        "• Now: R + HTML/JS. Next (where helpful): Python/JS widgets")
    ...
    Call FillSlideContent(slide, _
        ...
        "• Metadata on each item: Bloom, PRIMM, Scaffolding" & vbCrLf & _
    ...
    Call FillSlideContent(slide, _
        ...
        "Task:" & vbCrLf & _
        "• Compute rate per 100,000 by district and flag above national average" & vbCrLf & vbCrLf & _
        "Skills:" & vbCrLf & _
        "• group_by() > summarise() > mutate() > arrange()" & vbCrLf & vbCrLf & _
        "Checks:" & vbCrLf & _
        "• denominator, missing values, sorting" & vbCrLf & vbCrLf & _
        "Output:" & vbCrLf & _
        "• APA table + 3-sentence interpretation" & vbCrLf & _
    ...
    Call FillSlideContent(slide, _
        ...
        "Task:" & vbCrLf & _
        "• Offense type x Gender (association)" & vbCrLf & vbCrLf & _
        "Steps:" & vbCrLf & _
        "• contingency > expected counts > Chi-square > effect size" & vbCrLf & vbCrLf & _
        "Traps:" & vbCrLf & _
        "• p-value vs. effect size; small cells" & vbCrLf & vbCrLf & _
        "Feedback:" & vbCrLf & _
        "• when needed, collapse categories or suggest Fisher's exact" & vbCrLf & _
    ...
    Call FillSlideContent(slide, _
        ...
        "• APA table: final formatted output (screenshot)" & vbCrLf & _
    ...
    Call FillSlideContent(slide, _
        ...
        "• Already delivered: Dodona items • synthetic data • APA pipeline" & vbCrLf & _
        "• Domain-authentic: criminology tasks; SPSS>R bridges; thesis-ready outputs" & vbCrLf & _
        "• Structured, evidence-based development: clear specs, tests, and use of progress data to improve items" & vbCrLf & _
        "• Operational readiness: concrete plan to Dec 15; GitHub releases; content freezes" & vbCrLf & _
        "• Department fit: 3 years in your workflows; collaborations with course leads")
    
    ' Fill Slide 11: Dutch Delivery & Quality (17:00–18:30)
    Set slide = pres.Slides(11)
    Call FillSlideContent(slide, _
        "Dutch Delivery & Quality", _
        "• NT2 A2, currently A3 (CVO); prototype delivered in Dutch" & vbCrLf & _
        "• Review process: NL<>EN glossary • DeepL/LanguageTool • native speaker review" & vbCrLf & _
        "• Readability: B1 target; student clarity polls (Dodona/Ufora)" & vbCrLf & _
        "• KPIs: >=95% items pass B1 checks; >=4/5 clarity rating")
    
    ' Fill Hidden Slide 12: Reserve Q&A
    Set slide = pres.Slides(12)
    Call FillSlideContent(slide, _
        "Reserve Q&A", _
        "• SPSS > R quick map (Select Cases > filter, Compute > mutate, Crosstabs > count/tabyl)" & vbCrLf & _
    ...
    MsgBox "Template filled successfully! Saved as Interview_presentation_FILLED.pptx" & vbCrLf & _
           "11 slides created with exact content specification", vbInformation
End Sub
...
Sub AddExactSpeakerNotes(pres As Object)
    ...
    If pres.Slides.Count >= 1 Then
        speakerText = "Thanks for the time. I'll cover 1) the challenge and my teaching approach, 2) concrete examples and tools, and 3) the Sep-Dec plan and impact—then I'll close with why I fit this role and how I guarantee Dutch-language quality."
    ...
    If pres.Slides.Count >= 2 Then
        speakerText = "Grounding every step in criminology reduces anxiety, while reproducible outputs build confidence."
    ...
    If pres.Slides.Count >= 3 Then
        speakerText = "These pillars keep cognitive load manageable and make learning feel relevant."
    ...
    If pres.Slides.Count >= 4 Then
        speakerText = "This setup lets us deliver quickly, keep content tidy, and support different learning styles."
    ...
    If pres.Slides.Count >= 5 Then
        speakerText = "These are the core components this role needs: authentic tasks, targeted hints, and clean reporting."
    ...
    If pres.Slides.Count >= 6 Then
        speakerText = "This targets classic pitfalls—wrong denominators and NA handling—and finishes with clear, publishable output."
    ...
    If pres.Slides.Count >= 7 Then
        speakerText = "Students practice correct inference language and how to adapt when assumptions are strained."
    ...
    If pres.Slides.Count >= 8 Then
        speakerText = "This is exactly what students see. I'm happy to open a short example if useful."
    ...
    If pres.Slides.Count >= 9 Then
        speakerText = "Two content freezes and a short pilot protect quality and give us time to tidy details."
    ...
    If pres.Slides.Count >= 10 Then
        speakerText = "I'm proposing something we can deliver now because much of it is already built and aligned with how we work."
    ...
    If pres.Slides.Count >= 11 Then
        ...
        "Dutch: Ik ontwikkel de leerlijnen in het Nederlands. Mijn niveau is NT2 A2 en ik volg A3 bij CVO. Ondanks mijn beperkte spreekvaardigheid heb ik het prototype al in het Nederlands opgeleverd. Met een vaste review-procedure (terminologielijst, automatische checks, review door een moedertaalspreker) borgen we duidelijke taal vanaf dag een."
        pres.Slides(11).NotesPage.Shapes.Placeholders(2).TextFrame.TextRange.Text = speakerText
    End If
    
    ' Hidden Slide 12: Reserve Q&A
    If pres.Slides.Count >= 12 Then
        speakerText = "Hidden for slide show; available if questions go deep."
